<script lang="ts">
  ...
  async function checkForUpdates() {
    try {
      ...
      updateInfo = updateData.body;
      
    } catch (err) {
      console.warn('Failed to check for updates:', err);
      // Don't show error for update check failure, just log it
    } finally {
      isCheckingUpdate = false;
    }
  }

  // Install update (single step like loa-logs-master)
  async function installUpdate() {
    try {
      ...
      showError(`Failed to install update: ${err}`);
    } finally {
  ...
  async function loadChangelogData() {
    try {
      ...
      showError(`Failed to load changelog data: ${err}`);
    } finally {
      ...
      <div class="settings-section">
        <div class="section-header">
          ...
          <div>
            ...
            <p>Check for new versions and view changelog information</p>
          </div>
        </div>

        <div class="updates-changelog-grid">
          <div class="update-info">
            ...
            <button class="check-update-button" on:click={checkForUpdates} disabled={isCheckingUpdate}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M23 4v6h-6"/>
                <path d="M1 20v-6h6"/>
                <path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/>
            ...
            {#if updateAvailable && !isInstallingUpdate && !isCheckingUpdate}
              <button class="install-update-button" on:click={installUpdate}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/>
          ...
          <!-- Changelog & Known Issues Section -->
          <div class="changelog-section">
            <div class="changelog-header">
              ...
              <div class="changelog-content">
                <h4>Changelog & Known Issues</h4>
                <p>View recent updates and known bugs/issues</p>
              </div>
            </div>
            ...
            <div class="changelog-content">
              ...
              {:else if changelogs && changelogs.versions}
                  ...
                  <div class="changelog-item">
                    ...
                    <div class="changes">
                      {#each version.changes as change}
                        <div class="change-item {change.type.toLowerCase()}">
                ...
                {#if knownBugs && knownBugs.bugs}
                  <div class="known-issues">
                    <h5>Known Issues</h5>
                    {#each knownBugs.bugs as bug}
                      <div class="issue-item">
                        <div class="issue-status {bug.severity.toLowerCase()}">{bug.severity}</div>
...
<style>
  ...
  .update-available, .up-to-date, .checking-updates {
    display: flex;
    align-items: flex-start;
    gap: 12px;
    padding: 16px;
  ...
  .up-to-date .update-icon {
    background: var(--md-sys-color-tertiary);
    color: var(--md-sys-color-on-tertiary);
  }

  .checking-updates .update-icon {
    background: var(--md-sys-color-surface-variant);
  }
