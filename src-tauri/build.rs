fn main() {
    ...
    let tauri_config = std::fs::read_to_string("tauri.conf.json").expect("Failed to read tauri.conf.json");

    // Parse the JSON to extract version
    let config: serde_json::Value = serde_json::from_str(&tauri_config).expect("Failed to parse tauri.conf.json");
