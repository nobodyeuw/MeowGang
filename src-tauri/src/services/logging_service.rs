...
impl LoggingService {
    ...
    pub fn get_log_content(&self) -> Result<String> {
        ...
        std::fs::read_to_string(log_file_path).map_err(|e| anyhow::anyhow!("Failed to read log file: {}", e))
    }
    ...
    pub fn clear_log(&self) -> Result<()> {
        ...
        std::fs::write(&log_file_path, "").map_err(|e| anyhow::anyhow!("Failed to clear log file: {}", e))?;
...
fn get_log_directory() -> Result<PathBuf> {
    ...
    let app_data = dirs::data_dir().ok_or_else(|| anyhow::anyhow!("Failed to get app data directory"))?;

    let log_dir = app_data.join("LOA Tracker").join("logs");
