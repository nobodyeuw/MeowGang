...
impl GoldLoggingService {
    ...
    pub fn process_pending_gold_logs(&self, raid_state: &crate::state::RaidDataState) -> Result<usize> {
        ...
        for (char_id, content_id, difficulty, timestamp, session_id) in pending_rows {
            ...
            let raid_data =
                match self.get_raid_data_from_state(&content_id, &normalized_difficulty, &session_id, raid_state) {
                    Ok(data) => data,
                    Err(e) => {
                        crate::log_warn!(
                            "Failed to get raid data for {} {} from state: {}",
                            content_id,
                            difficulty,
                            e
                        );
            ...
            pending_entries.push((
                char_id, content_id, difficulty, gate, timestamp, raid_data, take_gold, buy_box,
            ));
        }
        ...
        if !pending_entries.is_empty() {
            ...
            for (char_id, content_id, difficulty, gate, timestamp, raid_data, take_gold, buy_box) in pending_entries {
                crate::log_debug!(
                    "Processing gold for char {} raid {} gate {} take_gold {} buy_box {}",
                    char_id,
                    content_id,
                    gate,
                    take_gold,
                ...
                if let Err(e) = self.log_gold_for_raid_completion_in_transaction(
                    ...
                    crate::log_error!(
                        "Failed to log gold for character {} raid {}: {}",
                        char_id,
                        content_id,
                        e
                    );
    ...
    fn log_gold_for_raid_completion_in_transaction(
        ...
        // Find gate data
        let gate_data = raid_data
            .gates
            .iter()
            .find(|g| g.gate == gate)
            .ok_or_else(|| anyhow::anyhow!("Gate {} not found for raid {}", gate, raid_id))?;

        let weekly_reset: i64 = tx
            .query_row("SELECT last_weekly_reset FROM app_state LIMIT 1", [], |row| row.get(0))
            .unwrap_or(0);
    ...
    /// Get raid data from RaidDataState (frontend-driven, NO backend hardcoding)
    fn get_raid_data_from_state(
        &self,
        content_id: &str,
        details: &str,
        session_id: &str,
        raid_state: &crate::state::RaidDataState,
    ) -> Result<Raid> {
        ...
        let raid = raid_state.find_raid(raid_id, difficulty).ok_or_else(|| {
            anyhow::anyhow!(
                "Raid {} with difficulty {} not found in RaidDataState",
                raid_id,
                difficulty
            )
        })?;
        ...
        // Find matching gate
        let gate_data = raid.gates.iter().find(|g| g.gate == gate).ok_or_else(|| {
            anyhow::anyhow!(
                "Gate {} not found for raid {} with difficulty {}",
                gate,
                raid_id,
                difficulty
            )
    ...
    pub fn delete_gold_logs_for_raid_completion(
        ...
        crate::log_info!(
            "Deleting gold logs for char {} raid {} difficulty {}",
            char_id,
            content_id,
            difficulty
        );
    ...
    pub fn clean_duplicate_gold_logs(&self, char_id: i64) -> Result<usize> {
        ...
        crate::log_info!(
            "Cleaned up {} duplicate gold log entries for character {}",
            result,
            char_id
        );
...
mod tests {
    ...
    fn test_clean_all_duplicate_gold_logs_removes_duplicates() {
        ...
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM gold_logs", [], |r| r.get(0))
            .unwrap();
        assert_eq!(count, 1);
    }
    ...
    fn test_clean_duplicate_gold_logs_per_character() {
        ...
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM gold_logs WHERE char_id = 2", [], |r| r.get(0))
            .unwrap();
        assert_eq!(count, 1);
    }
    ...
    fn test_delete_gold_logs_for_raid_completion() {
        ...
        let deleted = service
            .delete_gold_logs_for_raid_completion(1, "act_3_mordum", "Hard", "act_3_mordum_Gate 1")
            .unwrap();
        ...
        let remaining: i64 = conn
            .query_row("SELECT COUNT(*) FROM gold_logs WHERE notes LIKE '%Armoche%'", [], |r| {
                r.get(0)
            })
            .unwrap();
