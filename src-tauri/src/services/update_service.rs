...
impl UpdateService {
    pub async fn check_for_updates(current_version: &str) -> Result<UpdateInfo> {
        ...
        let response = client
            .get("https://raw.githubusercontent.com/nobodyeuw/MeowGang-Tracker/main/latest.json")
        ...
        let update_available = match (Version::parse(current_version_str), Version::parse(latest_version_str)) {
        ...
        loop {
            match Self::try_download_and_install(&download_url).await {
                ...
                Err(e) if retry_count < MAX_RETRIES => {
                    crate::log_warn!("Download failed, retrying ({}/{}): {}", retry_count + 1, MAX_RETRIES, e);
                    retry_count += 1;
    ...
    async fn try_download_and_install(download_url: &str) -> Result<()> {
        ...
        if bytes.len() < 1024 * 1024 {
            return Err(anyhow::anyhow!(
                "Downloaded file too small ({}MB), likely corrupted",
                bytes.len() / (1024 * 1024)
            ));
        }
        ...
        {
            match Command::new(&file_path).arg("/S").spawn() {
                Ok(_) => {
