...
pub async fn get_weekly_gold_stats(
    ...
    crate::log_debug!(
        "get_weekly_gold_stats: week_start={}, summary={:?}",
        week_start,
        summary
    );
...
pub async fn delete_gold_logs_for_raid(
    ...
    match gold_service.delete_gold_logs_for_raid_completion(char_id, &content_id, &difficulty, &session_id) {
        Ok(count) => Ok(format!(
            "Deleted {} gold log entries and completion status for raid {}",
            count, content_id
        )),
...
#[tauri::command]
pub async fn clean_all_duplicate_gold_logs(gold_service: State<'_, GoldLoggingService>) -> Result<String, String> {
