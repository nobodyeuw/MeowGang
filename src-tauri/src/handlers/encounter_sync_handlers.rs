...
pub fn sync_encounters_to_completions(
    ...
    sync_encounters_to_completions_internal(app, todo_repo.inner().clone(), settings_manager.inner())
}
...
fn get_cleared_encounters(
    ...
    crate::log_debug!(
        "Filtering encounters since weekly_reset_ts: {} (milliseconds)",
        weekly_reset_ts
    );
...
fn process_encounter(
    ...
    let mapping = match boss_mapper.map_boss_to_encounter(&encounter.current_boss) {
        ...
        None => {
            crate::log_debug!(
                "Skipping encounter: Boss '{}' not found in boss_mapping",
    ...
    let char_id = match find_character_id_by_name(todo_repo, &encounter.local_player)? {
        ...
        None => {
            crate::log_debug!(
                "Skipping encounter: Character '{}' not found in database",
